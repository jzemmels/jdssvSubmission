...
```{r}
plt <- x3pPlot(reference_eroded,target_eroded,type = "list") %>%
  map(~ {
...
ggsave(plt[[1]],filename = "referenceScanExample.png")
ggsave(plt[[2]],filename = "targetScanExample.png")

knitr::plot_crop("referenceScanExample.png")
knitr::plot_crop("targetScanExample.png")
```

```{r}
load("../data/compData_fullScans.RData")

dat <- compData_fullScans <-
  map_dfr(seq(-3,3,by = 3),
...
knitr::plot_crop("cellGridExample.png")
```
...
dat <- compData %>%
  group_by(cellIndex,direction) %>% 
  filter(direction == "comparison_refToTarget" & fft_ccf == max(fft_ccf)) %>%
  filter(cellIndex %in% c("2, 2","8, 3")) %>%
  ungroup() %>%
  select(cellIndex,theta,cellHeightValues,alignedTargetCell) %>%
  pmap(~ {
    
    x3pPlot(..3,..4,type = "list") %>%
      map(~ {
...
ggsave(filename = "referenceCellExample.png",plot = plt1)
ggsave(filename = "alignedTargetCellExample.png",plot = plt2)

knitr::plot_crop("referenceCellExample.png")
knitr::plot_crop("alignedTargetCellExample.png")
```
