...
Consider \autoref{fig:filterLabeling} that shows the filtered element-wise average $\mathcal{F}_{|A - B^*| \leq \tau}\left(\frac{1}{2}(A + B^*)\right)$ on the left and the associated $cond$ matrix $|A - B^*| > \tau$ visualized in black-and-white in the middle with filtered elements, whose $cond$ value is $TRUE$, shown in white.
...
These models predicts the probability that a pair of cartridge cases match.
...
Models that are trained to maximize accuracy on imbalanced data often exhibit a "preference" for classifying new observations as the majority class, which in our case are non-matches \citep{Fernndez2018}.
...
As such, the left side shows a zoomed-in version of the top left corner of ROC curve plot, which makes it easier to compare the different curves.
...
The AUC for the All ACES logistic regression and random forest models are extremely similar, although the AUC for the logistic regression model is about 0.0003 higher than that of the random forest model.
Both of these are higher than the AUC of the two classifiers trained on the $C_0$ + Registration feature set.
...
We see that the All ACES, logistic regression model as the lowest equal error rate out of the four models with the All ACES, random forest model a close second.
...
This indicates that the models' ability to distinguish between non-matching comparisons generalizes well to the testing data.
...
The testing classification accuracy associated with this classifier is 94.20% with true negative and true positive rates of 94.37% and 97.32%, respectively.

The second row of \autoref{fig:trainTestAccuracy} summarizes results from training the three classifier models on a subset of the full ACES feature set consisting of the Cluster Indicator feature $C_0$ and the six registration-based features summarized in \autoref{tab:registrationFeatures}.
...
If we compare the "$C_0$ + Registration"-trained models in the second vs. the "All ACES"-trained models in the third row, we see that the addition of the other ACES features leads to improved test true negative and true positive rates (and consequently overall accuracy) with the largest gains observed in the true positive rates.

Across all five models, the All ACES-trained logistic regression model has the largest overall accuracy, true negative, and true positive rates of 97.71%, 97.83%, and 96.20%, respectively.
...
\autoref{fig:testProbs} shows a dot plot of the predicted match probabilities for the 41,769 non-match and 3,181 match comparisons in the test set.
...
As we expect, few non-match comparisons have large match probabilities, which justifies the low false positive rate observed in \autoref{fig:trainTestAccuracy}.
...
We see that the false negative rate for firearm T of 25.1% far outweighs the error rates from other firearm pairs.
Further, the 88 false negative firearm T comparisons comprise 75.2% of the 117 false negative test comparisons and 2.8% of the 3,181 matching test comparisons.
...
Noting the log scale on which these points are plotted, we see that the top five features most important features consist of the density-based features $C$, $\Delta_{\text{trans}}$, and $C_0$ and registration-based correlation features $\overline{\text{cor}}_{\text{cell}}$ and $\text{cor}_{\text{full}}$.

```{r,out.width="\\textwidth",echo=FALSE,fig.pos="htbp",fig.cap="\\label{fig:rfVarImpPlt} Variable importance measures from fitting a random forest to the training data set, repeated 10 times under various random seeds. The top five features consist of three of the four density-based features, $C$, $\\Delta_{\\text{trans}}$, and $C_0$, and two of the registration-based features, $\\overline{\\text{cor}}_{\\text{cell}}$ and $\\text{cor}_{\\text{full}}$. Points are plotted on a log scale and vertically jittered for visibility.",out.width=".9\\textwidth"}
...
This is why we consider separate classifiers based on these features for the training and testing results shown in \autoref{fig:testProbs_byFirearm}.
...
The CMC method has the lowest false positive rate across the three methods, albeit at the expense of a very high false negative rate, indicating a tendency to over-classify pairs as non-matches.
In contrast, the $C_0$-based and All ACES logistic regression classifiers have more balanced false positive and false negative error rates.
The All ACES logistic regression model achieves the lowest overall error of the three methods while maintaining relatively balanced false positive and false negative error rates.
...
\hline
CMC method & 2.6  & 0.3 & 33.8 \\
Only $C_0$ feature & 5.8  & 5.8 & 5.3\\
ACES LR & 2.3 & 2.2 & 3.8 \\
\end{tabular}
\caption{Testing classification error, false positive, and false negative rates for four types of classifier models. The CMC method results are derived from the implementation available in \citet{cmcR}. The "Only $C_0$ feature" classifier is analogous to the classification rule used in \citet{zhang_convergence_2021}. The last two rows show results from the Logistic Regression classifier trained on the all 19 ACES features under two optimization criteria - balancing the true negative and true positive rates or maximizing the overall classifciation accuracy on the training data set.}
...
For example, if we were to shift the similarity score classification threshold for the All ACES logistic regression model to maximize the overall accuracy on the training data, the resulting score threshold is 0.60 with test accuracy, true negative, and true positive rates of 99.4%, 99.9%, and 92.2%.
...
), levels = c("$C_0$", "$C_0$ + Registration", "All ACES"), class = "factor"), 
    Model = structure(c(1L, 3L, 4L, 3L, 4L), levels = c("Baseline", 
    "CART", "RF", "LR"), class = "factor"), Accuracy = c("94.67", 
    "97.09", "97.33", "\\textbf{98.19}", "98.15"), `True Negative` = c("94.37", 
    "97.09", "97.33", "\\textbf{98.19}", "98.15"), `True Positive` = c("97.32", 
    "97.09", "97.32", "\\textbf{98.18}", "98.14")), class = "data.frame", row.names = c(NA, 
-5L)) %>%
...
"All ACES"), class = "factor"), Accuracy = c("94.19", "96.79", 
"97.65", "96.97", "\\textbf{97.71}"), `True Positive` = c("94.74", 
"93.93", "95.42", "94.03", "\\textbf{96.20}"), `True Negative` = c("94.15", 
"97.00", "97.82", "97.18", "\\textbf{97.83}")), class = "data.frame", row.names = c(NA, 
-5L)) %>%
  select(`Feature Set`,Model,Accuracy,`True Negative`,`True Positive`) %>%
  arrange(`Feature Set`,Model) %>%
...
\jz{[Figure of fitted CART model here.]}
