...
We calculate the average and standard deviation of filtered correlation values across all cells:
\begin{align*}
\overline{cor}_{filt,\cdot} &= \frac{1}{T} \sum_{t=1}^T cor_{filt,t} \\
s_{cor_{filt}}^2 &= \frac{1}{T-1} \sum_{t=1}^T (cor_{filt,t} - \overline{cor}_{filt,\cdot})^2.
\end{align*}
...
More precisely, the algorithm returns a set of sets $\pmb{B} = \{B_1,B_2,...,B_L\}$ where each $B_l$ is a set of indices of the matrix $|A - B^*| < \tau$ that have a value of $TRUE$ and are connected by a chained-together sequence of 4 (Rook's) neighborhoods.
The right side of \autoref{fig:filterLabeling} shows each $B_l$ distinguished by different fill colors, $l = 1,...,L$.
...
```{r,out.width="\\textwidth",echo=FALSE,fig.pos="htbp",fig.cap="\\label{fig:filterLabeling} Labeled filter neighborhoods."}
