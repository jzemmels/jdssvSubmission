...
An examiner places the two cartridge cases beneath a "comparison microscope" that merges the views of two compound microscopes into a single split view, similar to what is shown in \autoref{fig:examination-algorithm-comparison}.
The examiner assesses the degree of similarity between the markings on the cartridge cases and decides whether the cartridge cases share share a common source or that the evidence is "inconclusive" \citep{AFTE1992}.
...
knitr::include_graphics("images/Cartridge on Sensofar Scope/examination_vs_algorithm_comparison_twoScansSideView.png")
```
...
We use 510 cartridge cases collected as part of a study by \citet{baldwin} who obtained 25 new Ruger SR9, 9mm Luger centerfire pistols and 20,000 Remington 9mm pistol cartridges.
The authors collected 800 fired cartridges from each handgun using a "brass catcher," a soft mesh box with an opening to catch fired cartridges after ejecting from the pistol.
...
\citet{baldwin} aimed to measure the false negative and false positive error rates of 218 forensic examiners.
...
For each comparison, examiners were provided 15 sets of three known-match cartridge cases and a single unknown-source cartridge case.
...
From the full-scan and cell-based registration procedures, we obtain six features summarized in \ref{tab:registrationFeatures}.
...
\begin{table}[htbp]
\centering
\begin{tabular}{p{.15\linewidth} p{.2\linewidth} p{.2\linewidth} p{.2\linewidth}}
