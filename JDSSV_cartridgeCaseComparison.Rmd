...
Instead, the algorithm forms clusters based on whether the number of points within an $\epsilon > 0$ distance of a point exceeds some pre-defined threshold, $minPts > 1$.
...
We calculate four features from the density-based clustering procedure: \textbf{average DBSCAN cluster size} $C$, the \textbf{DBSCAN cluster indicator} $C_0$, and the \textbf{root squared sum of the density-estimated registrations} $(\Delta_\theta, \Delta_{\text{trans}})$ defined as:
\begin{align*}
...
Consider \autoref{fig:filterLabeling} that shows the filtered element-wise average $\mathcal{F}_{|A - B^*| < \tau}\left(\frac{1}{2}(A + B^*)\right)$ on the left and the associated $cond$ matrix $|A - B^*| < \tau$ visualized in black-and-white in the middle with filtered elements shown in white.
...
\autoref{tab:visualDiagnosticFeatures} summarizes the six features based on visual diagnostics.
This concludes our explanation of the ACES feature set.
Next, we use the 16 ACES features to train and test classifier models.

```{=tex}
\begin{table}[htbp]
\centering
\begin{tabular}{p{.11\linewidth} p{.7\linewidth}}
$cor_{\text{full},\text{diff}}$ & \textbf{Full-scan differences correlation} across both comparison directions \\
$\overline{cor}_{\text{cell},\text{diff}}$ & \textbf{Average cell-wise differences correlation} across all cells in both comparison directions \\
$\overline{|S|}_{\text{full}}$ & \textbf{Average full-scan neighborhood size} across both comparison directions \\
$s_{\text{full},|S|}$ & \textbf{Standard deviation of the full-scan neighborhood sizes} across both comparison directions \\
$\overline{|S|}_{\text{cell}}$ & \textbf{Average cell-wise neighborhood sizes} across all cells in both comparison directions \\
...
We randomly split the cartridge case data set into 10 firearms for training and 15 firearms for testing.
...
plt <- testAccuracy_plt %>%
  ...
  facet_grid(rows = vars(featureGroup),
             ...
             switch = "y") +
  theme_bw() +
  scale_x_continuous(expand = expansion(c(0,.07)),
                     limits = c(NA,1),
                     labels = scales::percent,
...
ACES Random Forest & 1.1 & 0.27 & 12.8
\end{tabular}
...
When we applied this implementation to the data set discussed in this paper, we obtained a test classification error of 2.77\%.
These results highlight the fact that the ACES algorithm is more effective than previously proposed methods, particularly in identifying truly matching cartridge case pairs.
...
The labeled neighborhood size features measure how extreme the differences are while the differences correlation features measure similarities amongst the differences.
