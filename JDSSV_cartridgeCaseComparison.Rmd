...
Critics of traditional firearm and toolmark comparisons cite a lack of foundational validity (NAS 2009, PCAST 2016). \cite{nas2009, pcast}
...
Cite Hare et al. \cite{aoas} as a parallel paper to this one applied to bullet data.

Cite Xiao Hui's project.

Cite CMC method as predominant method. \hh{don't forget about cmcR :)}
Broadly summarize cell-based comparison procedure and CMC method logic.
Also reference Zhang et al. (2020) DBSCAN paper \cite{dbscan} here. \hh{XXX I don't know what you mean by Zhang et al 2020}
...
Discuss Baldwin study here. \cite{baldwin}
...
Once read into an R environment, we use sequence of functions available in the packages [x3ptools] \cite{x3ptools} and [cmcR] \cite{cmcR} to preprocess the raw scans.
...
\begin{align*}
\overline{cor}_{\text{full}} = \frac{1}{2}\left(cor_{A,\text{full}} + cor_{B,\text{full}}\right).
\end{align*}
...
```{r,out.width=".6\\textwidth",echo=FALSE,fig.pos="htbp",fig.cap="\\label{fig:cellGridExample} A source scan separated into a grid of $8 \\times 8$ cells. Each source cell is compared to a target scan to estimate where it aligns best. We exclude cells containing only missing values (visualized here as gray pixels)."}
...
s_{cor}^2 &= \frac{1}{T_A + T_B - 1} \sum_{d \in \{A,B\}} \sum_{t=1}^{T_d} (cor_{d,t} - \overline{cor}_{\text{cell}})^2
\end{align*} 
...
the \textbf{average cell-based pairwise-complete correlation} to be large and
<!-- $s_{CCF}^2$,  -->
the \textbf{standard deviation of the cell-based pairwise-complete correlations} small for truly matching cartridge case pairs.
...
\begin{align*}
s_{\theta^*}^2 =  \frac{1}{T_A + T_B - 1} \sum_{d \in \{A,B\}} \sum_{t=1}^{T_d} (\theta^*_{d,t} - \bar{\theta}^*)^2 \\
s_{m^*}^2 =  \frac{1}{T_A + T_B - 1} \sum_{d \in \{A,B\}} \sum_{t=1}^{T_d} (m^*_{d,t} - \bar{m}^*)^2 \\
s_{n^*}^2 = \frac{1}{T_A + T_B - 1} \sum_{d \in \{A,B\}} \sum_{t=1}^{T_d} (n^*_{d,t} - \bar{n}^*)^2 
\end{align*} 
where 
\begin{align*}
...
\end{align*}
We expect the \textbf{sample variance of the cell-based estimated registration} values to be small.
...
\hline
$\overline{cor}_{\text{full}}$ & \textbf{Full-scan pairwise-complete correlation} after aligning the scans. \\
\hline
...
\hline
$s_{cor}^2$ & \textbf{Sample variance of the cell-based pairwise-complete correlation} after aligning source cells to the target matrix using the cross-correlation function \\
\hline
$s_{m^*}^2$ & \textbf{Sample variance of the cell-based vertical translations} \\
\hline
$s_{n^*}^2$ & \textbf{Sample variance of the cell-based horizontal translations} \\
\hline
$s_{\theta^*}^2$ & \textbf{Sample variance of the cell-based rotations} \\
\hline
\end{tabular}
...
For truly matching cartridge case pairs, we expect the \textbf{average DBSCAN cluster size} to be large, the DBSCAN \textbf{cluster indicator} to be 1, and the \textbf{difference in the density-estimated registrations} to be small.

From this DBSCAN procedure, we obtain four features summarized in \autoref{tab:dbscanFeatures}.
...
\hline
$C_0$ & \textbf{Cluster indicator} variable indicating the existence of DBSCAN clusters in both comparison directions \\
\hline
$\Delta_\theta$ & \textbf{Absolute sum of the density-estimated rotations} between both comparison directions  \\
\hline
$\Delta_{trans}$ & \textbf{Root sum of squares of the cluster-estimated translations} between both comparison directions \\
\hline
\end{tabular}
\caption{Three similarity features based on the density-based clustering procedure.}
...
s^2_{\text{full},|S|} &= \frac{1}{L_A + L_B - 1} \sum_{d \in \{A,B\}} \sum_{l=1}^{L_d} (|S_{d,l}| - \overline{|S|}_{\text{full}})^2
\end{align*}
where $|S_{d,l}|$ is the size of the set $S_{d,l}$.
We assume that the \textbf{average} and \textbf{sample variance of the filtered full-scan neighborhood sizes} will be small for truly matching cartridge cases.
...
We calculate the per-cell average and sample variance of the labeled neighborhood cell size:
\begin{align*}
\overline{|S|}_{d,t} &= \frac{1}{L_{d,t}} \sum_{l=1}^L |S_{d,t,l}| \\
s^2_{d,t,|S|} &= \frac{1}{L_{d,t} - 1} \sum_{l=1}^{L_{d,t}} (|S_{d,t,l}| - \overline{|S|}_{\text{cell},d,t})^2.
\end{align*}
...
\bar{s}^2_{\text{cell},|S|} &= \frac{1}{T_A + T_B} \sum_{d \in \{A,B\}} \sum_{t=1}^{T_d} s^2_{d,t,|S|}
\end{align*}
Again, we assume that the \textbf{average cell-wise neighborhood size} and the \textbf{average sample variance of the cell-wise neighborhood sizes} will be small for truly matching cartridge cases.
...
\hline
$s^2_{\text{full},|S|}$ & \textbf{Sample variance of the filtered full-scan neighborhood sizes} across both comparison directions \\
\hline
$\overline{|S|}_{\text{cell}}$ & \textbf{Average filtered cell-wise neighborhood sizes} across all cells in both comparison directions \\
\hline
$\bar{s}^2_{\text{cell},|S|}$ & \textbf{Average sample variance of the cell-wise neighborhood sizes} across all cells in both comparison direction \\
\hline
\end{tabular}
...
We compare the results of three classifiers: based on a logistic regression, a classification tree \cite{rpart}, and a random forest \citep{randomForest, breiman}. We make use of the testing/training infrastructure of the \pkg{caret} package \cite{caret}. 
...
We consider the performance of the three models under different subsets of the overall data, which provides insight into the importance of the various feature groups.
...
plt <- accuracyData %>%
  ...
  mutate(featureGroup = factor(featureGroup,
                               ...
                               labels = c("ACES Features + Others",
                                          ...
                                          "ACES Features without Registration-Based Features",
                                          "ACES Features without Cluster-Based Features",
                                          "ACES Features without Visual Diagnostic Features",
                                          "ACES with abs. cell-based reg.")),
         modelType = ifelse(modelType == "Random Forest","RF",
                            ifelse(modelType == "Logistic Regression","LR",
                                   "CART"))) %>%
  filter(featureGroup %in% c("ACES Features","ACES Features without Registration-Based Features",
                             "ACES Features without Visual Diagnostic Features",
                             "ACES Features without Cluster-Based Features")) %>%
...
knitr::include_graphics("images/cartModel_relevantFeatures.png")
```
...
testAccuracy_plt <- testAccuracy %>%
  ...
  mutate(featureGroup = factor(featureGroup,
                               ...
                               labels = 
                                 c("ACES + ",
                                   ...
                                   "ACES, no Registration",
                                   "ACES, no Cluster-Based",
                                   "ACES, no Visual Diag.",
  ...
  modelType = factor(modelType,levels = c("CART","Logistic Regression",
                                          "Random Forest"),
                     labels = c("CART","LR","RF"))) %>%
  filter(featureGroup %in% c("ACES Features","ACES, no Registration",
                                   "ACES, no Visual Diag.",
                                   "ACES, no Cluster-Based")) %>%
...
[Discuss importance of various feature groups]
...
We treat the existence of clusters, along with the size of of those clusters, as features for a classifier.

Our intention in fitting different classification models was to compare each model's strengths and weaknesses.
As observed previously, the random forest has the overall best accuracy out of the three models with the logistic regression classifier being a consistent close second.
...
Ethically, we might favor the model that makes the fewest false positive classifications since mis-classifying a truly non-matching cartridge case pair may incriminate an innocent individual.
...
Compare results to best CMC method (and Baldwin?).
Qualify that this *our* implementation of the CMC method
