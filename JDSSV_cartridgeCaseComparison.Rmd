...
The following sections detail the results of this cross-validation procedure.

# Results

## Training Results

\autoref{fig:trainingAccuracy} shows the cross-validation estimated accuracies, including the false positive and false negative error rates, for the CART and random forest models.
...
plt <- accuracyData %>%
  ...
  mutate(featureGroup = factor(featureGroup,
                               levels = c("All Features",
                                          "Non-discriminatory features removed",
                                          "Non-discriminatory + noClusterInd features removed",
                                          "Non-discriminatory + CMC features removed",
                                          "Non-discriminatory + Visual Diagnostic features removed"))) %>%
  arrange(featureGroup,modelType) %>%
  ggplot(aes(x = Accuracy,y = modelType)) +
  geom_point(size = 2) +
  geom_segment(aes(x=.96,xend=Accuracy,y=modelType,yend=modelType)) +
  facet_wrap(~featureGroup,ncol = 1) +
  theme_bw() +
  scale_x_continuous(limits = c(.96,.991),
                     expand = expansion(0),
...
#                align = c("|l","r|"),
#                caption = "Estimated out-of-bag model accuracy based on 10-fold cross validation repeated thrice per model.",
#                label = "trainingAccuracy",
#                col.names = c("Model Type","Feature Group","Accuracy"))
```

```{r,fig.align='center',fig.cap='\\label{fig:trainingAccuracy} Training classification accuracy for three models based on various subsets of the training data set features. These accuracies are estimated based on 10-fold cross validation repeated thrice. In general, the Classification and Regression Tree (CART) model performs poorest while the Random Forest performs best. Removal of the non-discriminatory features has little effect on the accuracy across models.',fig.pos="htbp",out.width=".8\\textwidth"}
...
testAccuracy_plt <- testAccuracy %>%
  ...
  mutate(featureGroup = factor(featureGroup,
                               ...
                               labels = 
                                 c("All Features",
                                   "Discriminatory",
                                   "Discrim. - noClusterInd",
                                   "Discrim. - Registration",
                                   "Discrim. - Visual",
                                   "CMC")
  ),
  modelType = factor(modelType,levels = c("CART","Logistic Regression",
                                          "Random Forest",
                                          "CMC with Song et al. (2018) params",
                                          "CMC with Song et al. (2014) params"))) %>%
  select(featureGroup,modelType,everything()) %>%
  group_by(errorType) %>%
  mutate(xstart = min(value) - .02*min(value))
...
plt <- testAccuracy_plt %>%
  ...
  facet_grid(rows = vars(featureGroup),
             ...
             scales = "free") +
  theme_bw() +
  scale_x_continuous(expand = expansion(c(.05,.05)),
                     labels = scales::percent,
