...
Models that are trained to maximize accuracy on imbalanced data often exhibit a "preference" for classifying new observations as the majority class \citep{Fernndez2018}, which in our case are non-matches.
...
\autoref{fig:trainTestAccuracy} summarizes the training and testing accuracy, true negative and true positive rates for 13 binary classifiers.
...
We first compare the training and testing results across the 13 models and three columns in \autoref{fig:trainTestAccuracy}.
...
The second and third rows of \autoref{fig:trainTestAccuracy} summarize results from training the three classifier models on a subset of the full ACES feature set consisting of the Cluster Indicator feature $C_0$ and the six registration-based features summarized in \autoref{tab:registrationFeatures}.
As discussed in the next section, we consider this subset of features to represent the features used in Congruent Matching Cells methods \citep{song_proposed_2013,zhang_convergence_2021}.
...
The fourth and fifth rows of \autoref{fig:trainTestAccuracy} summarize the classification results based on using all 19 ACES features.
...
For both of these logistic regression models, the random forest model trained under the same conditions is a close second (see \autoref{tab:testDataResults} for more details).
...
<!-- \autoref{fig:trainingAccuracy} shows the training-estimated accuracy, true negative, and true positive rates for various binary classifiers. -->
<!-- In the first row, we consider a baseline classifier based solely on the Cluster Indicator feature $C_0$. -->
<!-- Namely, if the DBSCAN algorithm finds clusters in the cell-based translations from both directions of a cartridge case comparison, then that pair is classified as a match. -->
<!-- This is analogous to the classification rule used in \citet{zhang_convergence_2021}. -->
<!-- The training classification accuracy associated with this classifier is 94.2% with approximately balanced true negative and true positive rates of 94.7% and 94.2%, respectively. -->

<!-- The second and third rows of \autoref{fig:trainingAccuracy} summarize results from training the three classifier models on a subset of the full ACES feature set consisting of the Cluster Indicator feature $C_0$ and the six registration-based features summarized in \autoref{tab:registrationFeatures}. -->
<!-- As discussed in the next section, we consider this subset of features to represent the features used in Congruent Matching Cells methods \citep{song_proposed_2013,zhang_convergence_2021}. -->
<!-- We distinguish the second and third rows by whether the AUC or accuracy was used as an optimization metric. -->
<!-- We see in the second row that the logistic regression (LR) and random forest (RF) models perform comparably well with overall accuracies of around 97%. -->
<!-- The classification tree (CART) model lags behind with an accuracy of around 94%, which is a pattern that persists in the other rows. -->
<!-- We also note that the true negative and true positive rates for these three models are approximately equal to each other, which is the effect of the -->
...
<!-- ```{r,fig.align='center',fig.cap='\\label{fig:trainingAccuracy} Training classification accuracy for random forest (RF), logistic  regression (LR), and classification tree (CART) models based on various subsets of the ACES feature set. We consider a classifier based on a single feature, the Cluster Indicator feature $C_0$, as a baseline. The inclusion of other ACES features results in a higher classification accuracy over the baseline with the random forest and logistic regression seeing comparably large gains.',fig.pos="htbp",out.width="\\textwidth"} -->
<!-- knitr::include_graphics("images/trainResults_plt.png") -->
<!-- ``` -->
...
<!-- knitr::include_graphics("images/testResults_plt.png") -->
<!-- ``` -->
...
There is precedent for random forest models to be robust classifiers for forensic evidence \citep{aoas,Park2019}.
...
"93.93", "95.42", "94.03", "\\textbf{96.2}"), `True Negative` = c("94.15", 
"97", "97.82", "97.18", "\\textbf{97.83}")), class = "data.frame", row.names = c(NA, 
-5L)) %>%
