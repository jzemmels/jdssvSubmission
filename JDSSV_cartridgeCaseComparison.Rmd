...
We also perform \autoref{alg:registration} with the roles of $A$ and $B$ reversed, meaning scan $A$ is aligned to $B$ to obtain $A^*$.

To accommodate these two comparison "directions," we introduce a new subscript $d = 1,2$ representing whether $B$ was aligned to $A$ or vice versa, respectively.
As such, we obtain two sets of sets of estimated registrations, $(m^*_d,n^*_d,\theta^*_d)$ and $CCF_{\max,d}$ for $d=1,2$.\footnote{In reality, the true aligning registrations in the two comparison directions are opposites of each other. However, because we compare discretely-indexed arrays using a nearest-neighbor interpolation scheme, the estimated registrations differ slightly.}
For $d = 1$, we then apply the registration transformation $(m^*_1,n^*_1,\theta^*_1)$ to $B$ to obtain $B^*$ and compute the pairwise-complete correlation, $cor_{\text{full},1}$, between $A$ and $B^*$.
We repeat this in the other comparison direction to obtain $cor_{\text{full},2}$ and average the two:
\begin{align*}
\overline{cor}_{\text{full}} = \frac{1}{2}\left(cor_{\text{full},1} + cor_{\text{full},2}\right).
\end{align*}
...
Because $A$ and $B^*$ are already partially aligned based on the course rotation grid $\pmb{\Theta}$, we compare each reference cell to $B^*$ using a new rotation grid of $\pmb{\Theta}'_1 = \{\theta^*_1 - 2^\circ, \theta^*_1 - 1^\circ,\theta^*_1,\theta^*_1 + 1^\circ,\theta^*_1 + 2^\circ\}$.
As before, the $d = 1$ subscript here refers to the comparison direction.

We now extend the surface matrix notation introduced previously to accommodate cells.
Let $A_{t}$ denote the $t$th cell of matrix $A$, $t = 1,...,T_1$ where $T_1$ is the total number of cells containing non-missing values (e.g., $T_1 = 38$ in \autoref{fig:cellGridExample}) in scan $A$ and let $(a_t)_{ij}$ denote the $i,j$-th element of $A_t$.

The cell-based comparison procedure is outlined in \autoref{alg:cellComparison}.

```{=tex}
\begin{algorithm}[H]
\KwData{Reference matrix $A$, target matrix $B^*$, cell grid size $R \times C$, and rotation grid $\pmb{\Theta}'_1$}
\KwResult{Estimated translations and $CCF_{\max}$ values per cell, per rotation}
Partition $A$ into a grid of $R \times C$ cells\;
Discard cells containing only missing values, leaving $T_1$ remaining cells\;
\For{$\theta \in \pmb{\Theta}'_1$}{
Rotate $B^*$ by $\theta$ to obtain $B^*_\theta$\;
\For{$t = 1,...,T_1$}{
Calculate $CCF_{\max, \theta, t, 1} = \max_{m,n} (a_t \star b^*_\theta)_{mn}$\;
Calculate translation $[m^*_{\theta,t, 1},n^*_{\theta,t, 1}] = \arg \max_{m,n} (a_t \star b^*_\theta)_{mn}$
}
}
\Return{$\pmb{F}_1 = \{(m^*_{\theta,t, 1},n^*_{\theta,t,1}, CCF_{\max,\theta,t, 1}, \theta) : \theta \in \pmb{\Theta}'_1, t = 1,...,T_1\}$}
\caption{Cell-Based Comparison Procedure}
\label{alg:cellComparison}
\end{algorithm}
```

Rather than exclusively returning the registration that maximizes the overall CCF as in \autoref{alg:registration}, \autoref{alg:cellComparison} returns the set $\pmb{F}_1$ of translations and CCF values for each cell and each rotation considered.
...
Just as with the whole-scan registration, we calculate the pairwise-complete correlation between each cell $A_t$ and a matrix $B_{\theta,t}^*$ of the same size extracted from $B^*_{\theta}$ after translating by $[m^*_{\theta,1},n^*_{\theta,1}]$.
From this we obtain a set of pairwise-complete correlations for each rotation: $\{cor_{\text{cell},\theta,t,1} : \theta \in \pmb{\Theta}'_1\}$.
This whole procedure is repeated using $B$ as the reference scan and $A^*$ as the target, resulting in registration set $\pmb{F}_2$ and pairwise-complete correlations $\{cor_{\text{cell},\theta,t,2} : \theta \in \pmb{\Theta}'_2\}$.

For $d = 1,2$ and $t = 1,...,T_d$, define the cell-wise maximum CCF and pairwise-complete correlation as: 
\begin{align*}
CCF_{\max,t,d} &= \max_{\theta} \{CCF_{\max,\theta,t,d} : \theta \in \pmb{\Theta}'_d\} \\
cor_{\text{cell},t,d} &= \max_{\theta} \{cor_{\text{cell},\theta,t,d} : \theta \in \pmb{\Theta}'_d\}
\end{align*}

We compute the following features using the correlation data: 
\begin{align*}
\overline{cor}_{\text{cell}} &= \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d} \sum_{t=1}^{T_d} cor_{\text{cell},t,d} \\
s_{cor}^2 &= \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d-1} \sum_{t=1}^{T_d} (cor_{\text{cell},t,d} - \overline{cor}_{\text{cell}, \cdot, \cdot})^2
\end{align*} 
...
For $d = 1,2$ and $t = 1,...,T_d$, define the per-cell estimated translations and rotation as: 
\begin{align*}
\theta^*_{t,d} &= \arg \max_{\theta} \{CCF_{\max,\theta,t,d} : \theta \in \pmb{\Theta}'_d\} \\
m^*_{t,d} &= m^*_{\theta^*_{t,d},t,d} \\
n^*_{t,d} &= n^*_{\theta^*_{t,d},t,d}
\end{align*} 

We compute the following additional features using the estimated cell translations and rotations: 
\begin{align*}
s_{\theta^*}^2 = \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d-1} \sum_{t=1}^{T_d} (\theta^*_{t,d} - \bar{\theta}^*_{\cdot,d})^2 \\
s_{m^*}^2 = \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d-1} \sum_{t=1}^{T_d} (m^*_{t,d} - \bar{m}^*_{\cdot,d})^2 \\
s_{n^*}^2 = \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d-1} \sum_{t=1}^{T_d} (n^*_{t,d} - \bar{n}^*_{\cdot,d})^2 
\end{align*} 
where $\bar{m}^*_{\cdot,d} = \frac{1}{T_d} \sum_{t=1}^{T_d} m^*_{t,d}$, $\bar{n}^*_{\cdot,d} = \frac{1}{T_d} \sum_{t=1}^{T_d} n^*_{t,d}$, and $\bar{\theta}^*_{\cdot,d} = \frac{1}{T_d} \sum_{t=1}^{T_d} \theta^*_{t,d}$.
...
\hline
$\overline{cor}_{\text{cell}}$ & \textbf{Average cell-based pairwise-complete correlation} after aligning reference cells to the target matrix using the cross-correlation function \\
\hline
$s_{cor}^2$ & \textbf{Sample variance of the cell-based pairwise-complete correlation} after aligning reference cells to the target matrix using the cross-correlation function \\
\hline
...
The left scatterplot shows the per-cell estimated translations $[m^*_{\theta,t,d}, n^*_{\theta,t,d}]$ for $\theta = 3^\circ$ when scan $A$ is used as reference and $B^*$ as target, resulting a cluster of size 14.
...
```{r,out.width=".8\\textwidth",echo=FALSE,fig.pos="htbp",fig.cap="\\label{fig:dbscanScatterplot} Cluster assignments based on the Density Based Spatial Clustering with Applications to Noise (DBSCAN) algorithm for estimated translations in two comparison directions. Using scan $A$ as reference results in a cluster of size 14 (left) compared to 13 when scan $B$ is used as reference (right). Noting the reversed axes in the right plot, the mean cluster centers in both directions are approximately opposites of each other. Points are jittered for visibility."}
...
For truly matching cartridge case pairs, we expect the \textbf{average DBSCAN cluster size} to be large and the \textbf{difference in the density-estimated registrations} to be small.
...
We first consider the correlation $cor_{\text{full},\text{filt},d}$ between the filtered matrices $\mathcal{F}_{|A - B^*| > \tau}(A)$ and $\mathcal{F}_{|A - B^*| > \tau}(B^*)$ when $d = 1$ and $\mathcal{F}_{|A^* - B| > \tau}(A^*)$ and $\mathcal{F}_{|A^* - B| > \tau}(B)$ when $d = 2$.
The average of these is used as a feature:
\begin{align*}
\overline{cor}_{\text{full},\text{filt}} = \frac{1}{2}\left(cor_{\text{full},\text{filt},1} + cor_{\text{full},\text{filt},2}\right).
\end{align*}
We assume that $\overline{cor}_{\text{full},\text{filt}}$ will be larger for truly matching $A$ and $B$ than non-matching.
...
As before, we extend this notation to accommodate cell comparisons $t = 1,...,T_d$ for $d = 1,2$ using subscripts: $cor_{filt,t,d}$.
For example, $cor_{\text{filt},1,1}$ is the correlation between cell filtered surface matrices $\mathcal{F}_{|A_1 - B_{\theta_1^*,1}^*| > \tau}(A_1)$ and $\mathcal{F}_{|A_1 - B_{\theta_1^*,1}^*| > \tau}(B_{\theta_1^*,1}^*)$ where $B_{\theta_1^*,1}^*$ is the alignment of cell $A_1$ in surface matrix $B^*$ that maximizes the CCF.
We calculate the sample mean 
<!-- and variance  -->
of the filtered correlation values across all cells and both directions:
\begin{align*}
\overline{cor}_{\text{cell},\text{filt}} &= \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d} \sum_{t=1}^{T_d} cor_{\text{filt},t,d}
\end{align*}
...
More precisely, the algorithm returns a set of sets $\pmb{S}_d = \{S_{1,d},S_{2,d},...,S_{L_d,d}\}$ where each $S_{l,d}$ is a set of indices of the $cond$ matrix that have a value of $TRUE$ and are connected by a chained-together sequence of 4 (Rook's) neighborhoods.
The right side of \autoref{fig:filterLabeling} shows each $S_{l,d}$ distinguished by different fill colors, $l = 1,...,L_d$.
...
\begin{align*}
\overline{|S|}_{\text{full}} &= \frac{1}{2} \sum_{d=1}^2 \frac{1}{L_d} \sum_{l=1}^{L_d} |S_{l,d}| \\
s^2_{\text{full},|S|} &= \frac{1}{2} \sum_{d=1}^2 \frac{1}{L_d - 1} \sum_{l=1}^{L_d} (|S_{l,d}| - \overline{|S|}_{\text{full}})^2
\end{align*}
where $|S_{l,d}|$ is the size of the set $S_{l,d}$.
...
Let $\pmb{S}_{t,d} = \{S_{t,d,1},...,S_{t,d,L_{t,d}}\}$ denote the set of labeled neighborhoods for a cell $t = 1,...,T_d$, $d = 1,2$.
\begin{align*}
\overline{|S|}_{\text{cell},t,d} &= \frac{1}{L_{t,d}} \sum_{l=1}^L |S_{t,d,l}| \\
s^2_{\text{cell},|S|,t,d} &= \frac{1}{L_{t,d} - 1} \sum_{l=1}^{L_{t,d}} (|S_{t,d,l}| - \overline{|S|}_{\text{cell},t,d})^2.
\end{align*}

We assume that the cell-based $\overline{|S|}_{\text{cell},t,d}$ and $s^2_{\text{cell},|S|,t,d}$ will be small, on average, for truly matching cartridge cases.
Consequently, we use the sample average of these as features:
\begin{align*}
\overline{|S|}_{\text{cell}} &= \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d} \sum_{t=1}^{T_d} \overline{|S|}_{\text{cell},t,d} \\
\bar{s}^2_{\text{cell},|S|} &= \frac{1}{2} \sum_{d=1}^2 \frac{1}{T_d} \sum_{t=1}^{T_d} s^2_{\text{cell},|S|,t,d}
\end{align*}
