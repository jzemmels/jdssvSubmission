...
```{r,include=FALSE}
trainingAccuracy <- readRDS("data/trainingAccuracy.rds")

plt <- trainingAccuracy %>%
...
```{r,include=FALSE}
testAccuracy <- readRDS("data/testAccuracy.rds")

testAccuracy_plt <- testAccuracy %>%
  mutate(Accuracy = 1 - error,
         truePos = 1 - falseNeg,
         trueNeg = 1 - falsePos) %>%
  select(modelLabel,featureGroup,Accuracy,truePos,trueNeg) %>%
  ...
  mutate(xstart = ifelse(errorType == "True~Pos.~Rate",
                         ...
                         .935),
         featureGroup = factor(featureGroup,
                               levels = c("C0",
                                          "C0 + Registration Features",
                                          "All ACES Features"),
                               labels = c('C[0]',
...
plt <-  testAccuracy_plt %>%
  ggplot(aes(x = value,y = modelLabel)) +
  geom_point(size = 2) +
...
For example, the logistic regression model parameters can be intuitively explained in terms of the estimated increase in odds of a match.
...
However, we believe that the ultimate decision of which algorithm will be put into practice needs to be made with the input of the wider forensic and legal communities.
