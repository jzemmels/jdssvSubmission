...
The AUC for the All ACES logistic regression and random forest models are extremely similar, although the AUC for the random forest model is about 0.0002 higher than that of the logistic regression model.
...
Noting the different axes used in the first to columns of the baseline classifier results, we see that this classifier has an overall accuracy of 74.39%, which is largely explained by the true negative rate of 71.64%.
Despite having the largest true positive rate across all five classifiers of 99.05%, the large class imbalance in the training data leads to the true negative rate having a larger impact on the overall accuracy.
These results indicate that the DBSCAN algorithm identifies a cluster for the mast majority of truly matching comparisons, yet also for many non-matching comparisons.
...
Across all five models, the All ACES-trained logistic regression model has the largest overall test accuracy and true negative of 97.92% and 98.07% and the largest test true positive rate among the non-baseline classifiers of 95.91%.
...
We see that the false negative rate for firearm T of 26.2% far outweighs the error rates from other firearm pairs.
Further, the 92 false negative firearm T comparisons comprise 73% of the 126 false negative test comparisons and 2.9% of the 3,181 matching test comparisons.
...
Together, we consider $C_0$ and the registration-based features a fusion of previously-proposed cartridge case comparison algorithms.
...
Conversely, the $C_0$-based classifier has the lowest false negative rate and a very high false positive rate, indicating a tendency to over-classify pairs as matches.
These complementary results explain why combining the $C_0$ and Registration-based features leads to a better balance between the false positive and false negative rates as shown in the second row of \autoref{fig:trainTestAccuracy}.
However, the All ACES logistic regression model achieves the lowest overall error of the three methods while maintaining relatively balanced false positive and false negative error rates.
...
Only $C_0$ feature & 28.0  & 29.8 & 2.5\\
ACES LR & 2.3 & 2.2 & 3.8 \\
\end{tabular}
...
structure(list(`Feature Set` = structure(c(1L, 2L, 2L, 3L, 3L
    ...
    "CART", "RF", "LR"), class = "factor"), Accuracy = c("74.39", 
    "96.52", "96.72", "98.14", "\\textbf{98.33}"), `True Negative` = c("71.64", 
    "96.51", "96.73", "98.14", "\\textbf{98.33}"), `True Positive` = c("\\textbf{99.05}", 
    "96.59", "96.63", "98.14", "98.32")), class = "data.frame", row.names = c(NA, 
-5L)) %>%
...
"All ACES"), class = "factor"), Accuracy = c("72.03", "95.8", 
"97.46", "95.8", "\\textbf{97.92}"), `True Positive` = c("\\textbf{97.47}", 
"93.25", "95.03", "93.48", "95.91"), `True Negative` = c("70.16", 
"95.99", "97.64", "95.98", "\\textbf{98.07}")), class = "data.frame", row.names = c(NA, 
-5L)) %>%
  select(`Feature Set`,Model,Accuracy,`True Negative`,`True Positive`) %>%
  arrange(`Feature Set`,Model) %>%
