...
Police arrive at the scene of a fatal shooting.
It's clear that the perpetrator has fled, but the police find spent 9mm \emph{cartridge cases} - a metal casing that houses the bullet, gunpowder, and primer - on the ground near the victim's body.
They hear on the radio that a suspect in possession of a handgun has been detained nearby.
How can the police determine whether this individual was the shooter?

Test fires are collected from the suspect's gun.
These "known source" cartridge cases, along with the "questioned" cartridge case found at the crime scene, are sent to a forensic laboratory for examination.
A forensic examiner inspects these cartridge cases for \emph{breech face impressions}, which are markings left on the base of the cartridge case when it comes into contact with the back wall, or "breech face," of a barrel during the firing process.
Using a microscope, the examiner assesses the similarity between the markings on the cartridge case surfaces.
The examiner's task is to conclude whether or not they originated from the same firearm.
...
The situation described above outlines a traditional firearm and toolmark evidence examination [cite Thompson book].
Critics of traditional comparisons cite a lack of foundational validity (NAS 2009, PCAST 2016). \cite{nas2009, pcast}
For example, many have pointed out that the range of conclusions put forth by the by Association of Firearm and Toolmark Examiners are [vague and recursively defined].
...
[Vorburger et al. (2009)] proposed using the cross-correlation function (CCF) to compare images or scans of cartridge case surfaces.
...
[Hare et al. (2017)] use the CCF, among other features, to compare scans of bullets.
[Tai and Eddy (2017)] developed a "fully automatic," open-source cartridge case comparison pipeline that compared cartridge case images using the CCF.

[Song (2013)] noted that two matching cartridge cases often share similar impressions in specific regions, so calculating the CCF between two full scans may not highlight their similarities.
Instead, [Song (2013)] proposed partitioning one cartridge case scan into a grid of "cells" and calculating the CCF between each cell and the other scan.
...
[Song (2013)] outlined the "Congruent Matching Cells" algorithm to determine the number of cells that agree on a particular registration.
...
A number of follow-up papers proposed alterations to the the original CMC method [Tong et al. (2015), Chen et al. (2017)].
[Zemmels et al. (2022)] introduce an open-source implementation of the CMC method in the cmcR R package [cite cmcR].
As an alternative to defining Congruent Matching Cells, [Zhang et al. (2020)] proposed using the Density-Based Spatial Clustering with Applications to Noise (DBSCAN) algorithm from \cite{dbscan} to determine the number of cells in agreement on a specific registration.
...
Analogously, [Hare et al. (2017)] developed a validation procedure to select parameters for their bullet comparison algorithm.
In this work, we introduce a novel procedure to learn and validate optimal parameters using a cross-validation procedure.

# Cartridge Case Data

We use 500 cartridge cases collected as part of a study by \citet{baldwin} where 80 cartridge cases were fired from each of 25 Ruger SR9 handguns.
...
A false negative error occurs when truly matching cartridge cases, meaning they were fired from the same firearm, are classified as non-matches.
...
However, it should be noted that "inconclusive" decisions were treated as correct, no matter the ground-truth nature of the cartridge case set.

We scanned the 500 cartridge cases using the Cadre 3D-TopMatch High Capacity Scanner.
Briefly, this scanner collected images of a gel pad into which the cartridge case surface is impressed under various lighting conditions.
Proprietary software that comes packaged with this scanner then combines these images into a regular 2D array called a \emph{surface matrix}.
...
Once read into an R environment, we apply a sequence of functions available in the R packages [x3ptools] \cite{x3ptools} and [cmcR] \cite{cmcR} to preprocess the raw scans.
...
\jz{[More discussion on alignment in the figure. Include why it's important to focus on similarities, measured with the correlation, etc., as well as differences to fully understand whether a pair of cartridge cases match. We're bound to find similarities between two cartridge cases if that's all we look for.]}
...
A major benefit of the CART and logistic regression models are their interpretable compared to the random forest model.
...
The ACES algorithm substantiates and combines the classification rules used by previously proposed cartridge case comparison algorithms while infusing their logic with additional nuance.
For example, [Zhang et al. (2020)] proposed a binary classifier using the DBSCAN algorithm: if a cluster is identified, then classify the cartridge case pair as a match and otherwise a non-match.
...
While a classifier based solely on $C_0$ would misclassify this pair, the ACES logistic regression model correctly classifies this pair as a non-match.

Cells identified as part of a DBSCAN cluster could be considered an alternative definition of a "Congruent Matching Cell" (CMC) as defined in [Song (2013)].
...
In the ACES algorithm, we measure agreement by the number of cells that are close to each other rather than to a reference registration.
...
[Compare results to our best CMC method results (and Baldwin?).]
