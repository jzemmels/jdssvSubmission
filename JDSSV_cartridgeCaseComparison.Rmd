...
x3pPlt_horizontal <- function (x3pList, legend.quantiles = c(0, 
                                                             0.01, 0.25, 0.5, 0.75, 0.99, 1), height.quantiles = c(0, 
                                                                                                                   0.01, 0.025, 0.1, 0.25, 0.5, 0.75, 0.9, 0.975, 0.99, 1), 
...
For simplicity, we assume that $A,B \in \mathbb{R}^{d \times d}$ for $d > 0$.\footnote{This assumption of equally-sized, square matrices is easily enforced by padding the matrices with additional missing values.
...
\begin{align*}
A \oplus_{NA} B = (a_{ij} \oplus_{NA} b_{ij})_{1 \leq i,j \leq d} = 
\begin{cases}
...
Denoted $(A \star B)$, the CCF between $A$ and $B$ is a 2D array of dimension $2d - 1 \times 2d - 1$ with the $m,n$-th element given by:
\begin{align*}
(a \star b)_{mn} = \sum_{i=1}^d \sum_{j=1}^d a_{mn} \cdot b_{i + m, j + n}
\end{align*} 
where $1 \leq m,n \leq 2d - 1$.
...
Let $A_{k}$ denote the $k$th cell of matrix $A$, $k = 1,...,T$ where $T$ is the total number of cells containing non-missing values (e.g., $T = 38$ in \autoref{fig:cellGridExample}) and let $(a_k)_{ij}$ denote the $i,j$-th element of $A_k$.
...
\For{$k = 1,...,T$}{
Calculate $CCF_{\max, \theta, k} = \max_{m,n} (a_k \star b^*_\theta)_{mn}$\;
Calculate translation $[m^*_{\theta,k},n^*_{\theta,k}] = \arg \max_{m,n} (a_i \star b^*_\theta)[m,n]$
}
}
\Return{$\pmb{F} = \{(m^*_{\theta,k},n^*_{\theta,k}, CCF_{\max,\theta,k}, \theta) : \theta \in \pmb{\Theta}', k = 1,...,T\}$}
...
From this we obtain a set of pairwise-complete correlations for each rotation: $\{cor_{\text{comp},\theta,k} : \theta \in \pmb{\Theta}'\}$.
For $k = 1,...,T$, define the cell-wise maximum CCF and pairwise-complete correlation as: \begin{align*}
CCF_{\max,k} &= \max_{\theta} \{CCF_{\max,\theta,k} : \theta \in \pmb{\Theta}'\} \\
cor_{\text{comp},k} &= \max_{\theta} \{cor_{\text{comp},\theta,k} : \theta \in \pmb{\Theta}'\}
\end{align*} We compute the following features using the correlation data: \begin{align*}
\overline{CCF}_{\max, \cdot} &= \frac{1}{T} \sum_{k=1}^T CCF_{\max,k} \\
\overline{cor}_{\text{comp}, \cdot} &= \frac{1}{T} \sum_{k=1}^T cor_{\text{comp},k} \\
s_{CCF}^2 &= \frac{1}{T-1} \sum_{k=1}^T (CCF_{\max,k} - \overline{CCF}_{\max, \cdot})^2  \\
s_{cor}^2 &= \frac{1}{T-1} \sum_{k=1}^T (cor_{\text{comp},k} - \overline{cor}_{\text{comp}, \cdot})^2
\end{align*} We expect $\overline{CCF}_{\max, \cdot}$, $\overline{cor}_{\text{comp}, \cdot}$ to be large and $s_{CCF}^2$, $s_{cor}^2$ small for truly matching cartridge case pairs.

For $k = 1,...,T$, define the estimated translations and rotation for $A_i$ as: \begin{align*}
\theta^*_i &= \arg \max_{\theta} \{CCF_{\max,\theta,k} : \theta \in \pmb{\Theta}'\} \\
m^*_i &= m^*_{\theta^*_i,k} \\
n^*_i &= n^*_{\theta^*_i,k}
\end{align*} We compute the following additional features using the estimated cell translations and rotations: \begin{align*}
s_{m^*}^2 = \frac{1}{T-1} \sum_{k=1}^T (m^*_{k} - \bar{m}^*_{\cdot})^2 \\
s_{n^*}^2 = \frac{1}{T-1} \sum_{k=1}^T (n^*_{k} - \bar{n}^*_{\cdot})^2 \\
s_{\theta^*}^2 = \frac{1}{T-1} \sum_{k=1}^T (\theta^*_{k} - \bar{\theta}^*_{\cdot})^2
\end{align*} where $\bar{m}^*_{\cdot} = \frac{1}{T} \sum_{k=1}^T m^*_{k}$, $\bar{n}^*_{\cdot} = \frac{1}{T} \sum_{k=1}^T n^*_{k}$, and $\bar{\theta}^*_{\cdot} = \frac{1}{T} \sum_{k=1}^T \theta^*_{k}$.
...
The left scatterplot shows the per-cell estimated translations $[m^*_{\theta,k}, n^*_{\theta,k}]$ for $\theta = 3^\circ$ when scan $A$ is used as reference and $B^*$ as target, resulting a cluster of size 14.
...
In particular, for a matrix $X \in \mathbb{R}^{d \times d}$ and condition $cond: \mathbb{R}^{d \times d} \to \{TRUE,FALSE\}^{d \times d}$, we define an element-wise filter operation $\mathcal{F}: \mathbb{R}^{d \times d} \to \mathbb{R}^{d \times d}$ as:
\begin{align*}
\mathcal{F}_{cond}(X) = 
(f_{ij})_{1 \leq i,j \leq d} =
\begin{cases}
...
plt <- impressions::x3p_comparisonPlot(
  x3p1 = scanA,
  x3p2 = scanBStar,
  plotNames = c("Scan A","Scan B*","Filtered Element-wise Average",
                "Scan A Filtered Differences","Scan B Filtered Differences"))

ggsave(plot = plt,filename = "figures/fullScan_comparisonPlot.png")
...
We now describe each assumption and their associated features.
