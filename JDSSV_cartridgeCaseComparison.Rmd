...
Depending on the outcome of this comparison, the forensic examiner decides if the cartridge cases share or do not share a common source or if there is "inconclusive" evidence to make the call.
...
\citet{pcast} push for "developing and testing image-analysis algorithms" to objectively measure the similarity between cartridge cases.
An automatic comparison algorithm could be used as part of an examination to supplement or inform an examiner's opinion \citep{swofford}.
...
We then discuss the data collection procedure used to obtain 510 cartridge scans used to train and validate the ACES algorithm.
To our knowledge, this is the largest published study of a cartridge case comparison algorithm to-date, with the next largest combining four different data sets totaling 195 cartridge cases \citep{chen_convergence_2017}.
After describing the ACES algorithm, we present summary results from training and testing three binary classifier models: a random forest, logistic regression, and classification and regression tree.
...
\citet{aoas} use the CCF, among other features, to compare scans of bullets.
...
As an alternative to defining Congruent Matching Cells, \citet{zhang_convergence_2021} proposed using the Density-Based Spatial Clustering with Applications to Noise (DBSCAN) algorithm from \cite{dbscan} to determine the number of cells in agreement on a specific registration.
...
Analogously, \citet{aoas} developed a validation procedure to select parameters for their bullet comparison algorithm.
...
We scanned the 510 cartridge cases using the Cadre 3D-TopMatch High Capacity Scanner.
...
We calculate features at two scales: between two whole scans and between individual cells similar to the CMC method.
...
\jz{[More discussion on alignment in the figure. Point out that it is important to consider features that measure the similarities and differences, since we're likely to find some similarities between non-matching scans]}
...
\jz{[Write full logistic regression model?]}
...
Cells identified as part of a DBSCAN cluster could be considered an alternative definition of a "Congruent Matching Cell" (CMC) as defined in \citep{song_proposed_2013}.
...
\citep{song_proposed_2013} measures the similarity between two cartridge cases using the total number of CMCs and proposes classifying a pair as matching if the CMC count exceeds five.
...
These are similar to manually-selected thresholds used across various CMC papers \cite{tong_improved_2015,chen_convergence_2017}.
...
Include why it's important to focus on similarities, measured with the correlation, etc., as well as differences to fully understand whether a pair of cartridge cases match. 
