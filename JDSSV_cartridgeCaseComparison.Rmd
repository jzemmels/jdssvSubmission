...
We calculate the sample mean and variance of the filtered correlation values across all cells:
\begin{align*}
...
\bar{s}^2_{|S|,\cdot} &= \frac{1}{T} s^2_{S, t} \\
s^2_{s^2_{|S|, t}} &= \frac{1}{T-1} \left(s^2_{|S|, t} - \bar{s}^2_{|S|,\cdot}\right)^2.
\end{align*}
Again, we assume that these four features will be small for truly matching cartridge cases.

\autoref{tab:visualDiagnosticFeatures} summarizes the 9 features calculated based on the visual diagnostics.
...
\hline
\end{tabular}
\caption{Eight similarity features calculated based on visual diagnostics.}
