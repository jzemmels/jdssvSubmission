...
[Vorburger et al. (2009)] proposed using the cross-correlation function (CCF) to compare 3D topographic scans of cartridge case surfaces.
...
Tai and Eddy (2017) developed a "fully automatic" cartridge case comparison pipeline that preprocessed and compared 2D images of cartridge case scans.

[Song (2013)] noted that two matching cartridge cases often share similar impressions in specific regions, meaning calculating the CCF between two full scans may not highlight their similarities.
...
[Zemmels et al. (2022)] provide an open-source implementation of the CMC method in the cmcR R package [cite cmcR].
...
logisticRegOddsFactors %>%
  slice(1:5) %>%
  mutate(logistOddsFactor = case_when(varName == "noClusterIndTRUE" ~ "$3.8 \\times 10^{47}$",
                             varName == "differenceCor_ave" ~ "1,242.0",
                             varName == "cell_theta_sd" ~ "67.1",
                             varName == "clustCenterDiff" ~ "1.8",
                             varName == "cell_x_sd" ~ "1.1",
                             TRUE ~ "")) %>%
  mutate(varName = case_when(varName == "noClusterIndTRUE" ~ "Cluster Indicator",
                             ...
                             varName == "cell_x_sd" ~ "Cell-Based Horizontal Translation SD",
                             TRUE ~ "")) %>%
